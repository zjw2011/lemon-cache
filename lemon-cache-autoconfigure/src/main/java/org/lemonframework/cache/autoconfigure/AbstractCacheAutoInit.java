...
import org.lemonframework.cache.anno.support.ConfigProvider;
...
public abstract class AbstractCacheAutoInit implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        if (!inited) {
            synchronized (this) {
                if (!inited) {
                    process("lemon.cache.local.", autoConfigureBeans.getLocalCacheBuilders(), true);
                    process("lemon.cache.remote.", autoConfigureBeans.getRemoteCacheBuilders(), false);
                    inited = true;
                }
            }
        }
